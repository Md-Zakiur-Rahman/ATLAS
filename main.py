...
def log_memory_usage():
    ...
    logger.info("ATLAS Memory Usage: %.2f MB", memory_mb)
...
def launch_dashboard():
    ...
    try:
        threading.Thread(target=dashboard_main, daemon=True).start()
...
if not ml_detector.is_initial_training_complete and not args.train:
    threading.Thread(target=_initial_training_watcher, daemon=True).start()
...
# =========================================================
# Launch Dashboard (Parallel, Non-Blocking)
# =========================================================
launch_flask_api()
launch_dashboard()
...
try:
    ...
    if args.train:
        logger.info("ML Training Mode Started")
